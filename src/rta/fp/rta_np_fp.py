...
class NonPreemptiveFixedPriorityRTA:
    def __init__(self, task_set):
        ...
        self.blocking_bound = {}
        for t in task_set:
            bound = max([tt.wcet-1 for tt in task_set 
                                    if tt.priority < t.priority], default=0)
            assert(bound >= 0)
    ...
    def max_busy_interval(self, task_set, tsk):
        f = lambda delta : self.blocking_bound[tsk.id] + fp.total_hep_rbf(task_set,tsk, delta)
