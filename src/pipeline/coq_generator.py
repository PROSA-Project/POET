...
def generate_proof (problem_instance, tsk, results, bounded_tardiness_allowed, split_declaration):
    ts = problem_instance.task_set
    proof = templates.get_main_certificate(problem_instance)
    proof = patch(proof, templates.WC_TASK_SET_DECLARATION, task_set_declaration(problem_instance))
    ...
    proof = patch(proof, templates.WC_F_SOLUTIONS, get_F_solutions(problem_instance, results.Fs))
    ...
    proof,_ = conditional_cut_patch(proof, templates.WC_DEADLINE_IS_RESPECTED_START,
                                           templates.WC_DEADLINE_IS_RESPECTED_END,
                                           use_tardiness_bound)
    proof,_ = conditional_cut_patch(proof, templates.WC_TARDINESS_IS_BOUNDED_START,
                                           templates.WC_TARDINESS_IS_BOUNDED_END, 
                                           not use_tardiness_bound)
    proof,_ = conditional_cut_patch(proof, templates.WC_DEADLINE_IS_RESPECTED_PRINT_START,
                                           templates.WC_DEADLINE_IS_RESPECTED_PRINT_END,
                                           use_tardiness_bound)
    proof,_ = conditional_cut_patch(proof, templates.WC_TARDINESS_IS_BOUNDED_PRINT_START,
                                           templates.WC_TARDINESS_IS_BOUNDED_PRINT_END, 
                                           not use_tardiness_bound)

    proof,declaration = conditional_cut_patch(proof, templates.WC_DECLARATION_START,
                                                     templates.WC_CERTIFICATE_START, split_declaration)
...
def task_set_declaration(problem_instance):
    ...
    for t in problem_instance.task_set:
        ...
        if t.task_type == TaskType.PERIODIC:
            task_dec = patch(task_dec, templates.WC_TASK_ARRIVAL, f"Periodic_T {t.period}%N")
        if t.task_type == TaskType.SPORADIC:
            task_dec = patch(task_dec, templates.WC_TASK_ARRIVAL, f"Sporadic_T {t.period}%N")
        elif t.task_type == TaskType.ARRIVAL_CURVE:
            task_dec = patch(task_dec, templates.WC_TASK_ARRIVAL, emax_declaration(t.arrival_curve))
