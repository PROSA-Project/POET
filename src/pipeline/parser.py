...
task_request_models = [TASK_PERIOD_TAG, TASK_MIN_INTERARRIVAL_TAG, TASK_PRIORITY_TAG, TASK_ARRIVAL_CURVE_TAG]

allowed_tags_at_root_level = [SCHEDULING_POLICY_TAG, PREEMPTION_MODEL_TAG, TASK_SET_TAG]
allowed_tags_at_task_level = [TASK_ID_TAG, TASK_WCET_TAG, TASK_DEADLINE_TAG] + task_request_models
...
def parse_file (file):
    ...
    try:
        with open(file, 'r') as stream:
            ...
            scheduling_policy = parse_alternatives(root, SCHEDULING_POLICY_TAG, pg.scheduling_policies)
            scheduling_policy = pg.process_synonyms(scheduling_policy)
            preemption_model = parse_alternatives(root, PREEMPTION_MODEL_TAG, pg.preemption_models)
...
def check_for_allowed_tags (father, allowed_tags):
    ...
    for key, _ in father.items():
        assert key in allowed_tags, f"'{key}' is an unrecognized tag. Allowed tags are: {utils.pretty_list(allowed_tags)}."
...
def parse_alternatives (father, tag, admitted_values):
    ...
    if value not in admitted_values:
        raise Exception(f"'{tag}' has value '{value}', which is not valid.\nUse one of these: {utils.pretty_list(admitted_values)}.")
    else:
        return value
...
def parse_arrival_curve (father, curve_tag):
    ...
    def parse_tuple (tuple):
        assert isinstance(tuple, list), "Each element of the curve must be a tuple of positive integers"
        ...
        try:
            ...
            raise Exception("Each tuple in a curve must only contain positive integers.")

        return (time, amount)

    curve = parse_required (father, curve_tag)
    
    assert isinstance(curve, list) and len(curve) == 2, "Curves must be of the form (h, [(t1,d1)] )."
    horizon = curve[0]
    steps = curve[1]

    assert isinstance(horizon, int), "Curves must be of the form (h, [(t1,d1)] )."
    assert isinstance(curve, list) and len(curve) > 0, "Curves must be of the form (h, [(t1,d1)] )."
...
def parse_task (task, expect_priority):
    ...
    assert has_period + has_min_interarrival + has_arrival_curve == 1, f"One of these should be specified: {utils.pretty_list(task_request_models)}."
    
    yes_no = " " if expect_priority else " not "
    assert has_priority == expect_priority, f"You should{yes_no}provide a priority for task with id {id}"

    if has_period or has_min_interarrival:
        assert has_wcet, "If a period or min interarrival is specified, a worst-case execution time is needed."
        
        period = parse_positive_number(task, TASK_PERIOD_TAG if has_period else TASK_MIN_INTERARRIVAL_TAG)
        ...
        assert has_wcet, "If an arrival curve is specified, a worst-case execution time or a request-bound function is needed."
