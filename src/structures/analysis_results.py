...
class TaskAnalysisResults:
    ...
    def __str__(self) -> str:
        exact_search_space = set((point for point in utils.search_space(self.SS) if point < self.L))
...
class AnalysisResults:
    ...
    def _analyze(self, rta, task):
        ...
        if self.problem_instance.scheduling_policy == FIXED_PRIORITY:
            ...
            if not all([F >= 0 for F in Fs]): every_F_solved = False
        else:  # EARLIEST_DEADLINE_FIRST
            for tsk_ss in SS:
                tsk_Fs = [rta.search_space_solution(task_set, task, A) for A in tsk_ss]
                if not all([F >= 0 for F in tsk_Fs]): every_F_solved = False
                Fs += [tsk_Fs]
    ...
    def all_deadlines_respected(self):
        ts = self.problem_instance.task_set
        return all([self.results[task].R > 0
                    and self.results[task].R <= task.deadline
                    for task in ts])
