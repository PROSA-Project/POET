...
def check_valid_curve(curve):
    ...
    if prev[0] == 0:
        assert prev[1] == 0, (
            f"{prev} : A window of zero must have zero generated workload."
        )
    for elem in curve[1:]:
        if prev is not None:
            assert prev[0] < elem[0], (
                f"{prev} - {elem} : The arrival times of the curve must be strictly monotonic."
            )
            assert prev[0] < elem[0], (
                f"{prev} - {elem} : The workload of the curve must increase at any point."
            )
        prev = elem


def check_valid_delta_min(delta_min):
    assert isinstance(delta_min, list) and len(delta_min) > 0, (
        "Min distance must be non-empty lists"
    )
